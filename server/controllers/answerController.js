...
exports.removeComment = async (req, res) => {
    try {
        ...
        if (!answerId || !commentId) return res.status(400).json({ message: 'answerId and commentId are required' });

        const ans = await Answer.findById(answerId);
        if (!ans) return res.status(404).json({ message: 'Answer not found' });

        const comment = ans.comments.id(commentId);
        ...
        res.json({ answer: populated, message: 'Comment deleted' });
    } catch (e) {
